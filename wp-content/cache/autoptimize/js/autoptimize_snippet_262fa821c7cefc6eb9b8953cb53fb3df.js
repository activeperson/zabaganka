...
SGPBPopup.playMusic=function(e){var args=e.detail;var popupId=parseInt(args.popupId);var options=SGPBPopup.getPopupOptionsById(popupId);var soundUrl=options['sgpb-sound-url'];var soundStatus=options['sgpb-open-sound'];if(soundStatus&&soundUrl&&!window.SGPB_SOUND[popupId]){var audio=new Audio(soundUrl);audio.play();window.SGPB_SOUND[popupId]=audio;}};SGPBPopup.prototype.initialsListeners=function()
...
{var instructions=popupData['sgpb-ShouldOpen'];instructions=decodeEntities(instructions);var F=new Function(instructions);return(F());}
popupConfig.customShouldClose=function()
{var instructions=popupData['sgpb-ShouldClose'];instructions=decodeEntities(instructions);var F=new Function(instructions);return(F());}
...
{var savedCookie=this.getPopupShowLimitationCookieDetails(popupData);var savedCookie=this.filterPopupLimitationCookie(savedCookie);return savedCookie;}
...
if((typeof cPageLevel=='boolean'&&cPageLevel==false)||cPageLevel==''||typeof cPageLevel=='undefined'){cookiePageLevel='path=/;';}
...
var value=cValue+((exDays==null)?';':'; '+expires+';'+cookiePageLevel);document.cookie=cName+'='+value;};SGPBPopup.isIE=function()
...
jQuery(this).bind('click',function(e){if(clickCount>1){return true;}
var attr=jQuery(e.originalEvent.target).parent().attr('data-popup-id');var attr2=jQuery(e.originalEvent.target).attr('data-popup-id');if(jQuery(e.originalEvent.target).closest('header').length&&(typeof attr=='undefined'||!attr)){if(typeof attr2=='undefined'||!attr2){return true;}}
...
jQuery(this).bind('hover',function(){if(hoverCount>1){return false;}
